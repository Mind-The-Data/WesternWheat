...
for i in array:
    ...
    som.train(n_job=1, verbose='info', train_rough_len=50, train_rough_radiusin=4, train_finetune_radiusin=1, train_finetune_len=50)
...
som = SOMFactory().build(df.values, mapsize=[20,20], normalization = 'var', initialization='pca', component_names=names,\
                    neighborhood = 'gaussian', lattice='rect')
som.train(n_job=1, verbose='info', train_rough_len=100, train_rough_radiusin=4, train_finetune_radiusin=1.5, train_finetune_len=50)
...
def HowManyK(k):
    ...
    for i in K:
        totalERROR = 0
        map_labels = som.cluster(n_clusters=i) # will eventually return more than labels.... 
        ...
        for j in range(1,pd.unique(df_labeled.cluster).size):
            ...
            # This isn't right, need to find distance!! PYTHAGOREON THEROEM dumb dumb....................
...
SSE_Matrix_avg = np.mean(SSE_Matrix, axis=0)

# SSE of K-means

plt.plot(np.arange(2,SSE_Matrix_avg.size+2), SSE_Matrix_avg)
plt.title('K-Means "elbow" plot')
...
map_labels, km = som.cluster(n_clusters=8)
# som._bmu[0]
